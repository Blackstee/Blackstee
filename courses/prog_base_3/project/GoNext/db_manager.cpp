...
int db_insertMusician(db_t * self, musician_t * mus)
{
    ...
    int rc = 0;
    if(SQLITE_OK != sqlite3_prepare_v2(self->db,"INSERT INTO Musicians  (Name, Surname, Style, Rating, Instruments, Salary) VALUES (?, ?, ?, ?, ?, ?);", -1, &stmt, NULL))
    ...
    sqlite3_bind_text(stmt, 3, mus->style,strlen(mus->style),SQLITE_TRANSIENT);
    sqlite3_bind_double(stmt, 4, mus->rating);
    sqlite3_bind_int(stmt, 5, mus->instruments);
    sqlite3_bind_int(stmt, 6, mus->salary);
...
musician_t * db_getMusicianByNumber(db_t * self, int number)
{
    ...
    sqlite3_stmt * stmt = NULL;
    sqlite3_prepare_v2(self->db, "SELECT * FROM Musicians WHERE Number = ?;", -1, &stmt, 0);
...
void db_updateMusician(db_t * self, musician_t * mus, int number)
{
    sqlite3_stmt * stmt = NULL;
    int rc = 0;
    if(SQLITE_OK != sqlite3_prepare_v2(self->db,"UPDATE Musicians SET Name = ?, Surname = ?, Style = ?, Rating = ?, Instruments = ?, Salary = ? WHERE Number = ?;", -1, &stmt, NULL)) {
...
{
    sqlite3_stmt * stmt = NULL;
    if(SQLITE_OK != sqlite3_prepare_v2(self->db, "DELETE FROM Musicians WHERE Number = ?;", -1, &stmt, 0))
        printf("It is not prepared!\n");
    sqlite3_bind_int(stmt, 1, number);
    int rc = sqlite3_step(stmt);
    if (SQLITE_DONE != rc) {
...
{
    sqlite3_stmt * stmt = NULL;
    sqlite3_prepare_v2(self->db, "SELECT COUNT(*) FROM Musicians;", -1, &stmt, 0);
    int rc = sqlite3_step(stmt);
    if (SQLITE_ERROR == rc) {
...
{
    sqlite3_stmt * stmt = NULL;
    const char * sqlQuery = "SELECT * FROM Musicians WHERE Instruments > ? AND Salary > ?;";
